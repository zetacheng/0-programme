...
## Dependency chain (critical path)
    ...
    P2-BETAV-CIRC-01            (SI-0  validate the extraction pipeline)
        ...
        → P2-MULTIPHASE-GRAV-01 (SI-2  ∀ phase healthy gravity?  PROGRAMME-DEATH)
        → P4-SEA-ICE-01         (SI-3  derived finite-tension interface?)
        → { P4-INTERFACE-DE-01           (SI-4  interface → dark energy)
            P4-WRINKLE-BRIDGE-01 → P4-BOUND-DM-01 }  (SI-5  wrinkle-bound DM)
...
| SI-2 | Paper 2 (primary), Paper 3 (dependency) | `P2-MULTIPHASE-GRAV-01` | **Programme-death.** Does *any* admissible stable phase produce healthy gravity? | every phase from SI-1; full `K_ij(p)` incl. all frozen-channel mixing; Paper 3 vector input | `∀ Φ: ¬HealthyGravity(Φ)` → **simplest lattice-fermion Sea–Ice programme terminated** |
...
| SI-5a | Paper 4 | `P4-WRINKLE-BRIDGE-01` | Is the SUPPORTED driven wrinkle (`P4-CL-002`) the same object as an equilibrium interface wrinkle? | driven pattern; SI-3 interface config space | Driven and equilibrium wrinkles are distinct objects → the DM programme may not inherit `P4-CL-002` as its background |
| SI-5b | Paper 4 (primary), Paper 5 (boundary) | `P4-BOUND-DM-01` | Does the derived wrinkle trap normalizable, long-lived, pressureless, clustering bound modes? | wrinkle background; fluctuation operator `O_fluct` | No such mode → wrinkle-bound DM rejected; may not revert to the Paper 5 continuum soliton |
...
- **Paper 4 driven wrinkle** (`P4-CL-002` SUPPORTED): wrinkles form and
  saturate in a driven EFT. Whether that is the *equilibrium* Sea–Ice
  wrinkle is exactly `P4-WRINKLE-BRIDGE-01` — unestablished, not assumed.
